...
contract("HBank", (accounts) => {
  let hamToken, hBank, oracle;

  // Sets the scene for borrowing
  const setTheScene = async () => {
    // Borrower needs to have some HAM tokens that she/he can give as a collateral
    // Keep in mind that the value of the collateral should be > requested Ether * colletaralization ratio
    // To earn HAM tokens, let's do a deposit (1 ETH) from the borrower's account first
    ...
    await hBank.deposit({ value: 10 ** 18, from: accounts[3] });
    await hBank.deposit({ value: 10 ** 18, from: accounts[4] });
    // Withdraw the deposit to earn HAM tokens (approx. 0.12 HAM tokens)
    await hBank.withdraw({ from: accounts[1] });

    // HBank should be able to transfer HAM tokens from the borrower to its own account
    // Thus, borrower should first call approve on HAMToken contract and set the allowance
    await hamToken.approve(
      hBank.address,
    ...
    // Since HBank needs to have some Ether in it to lend out,
    ...
    // Update the ETHHAMPriceFeedOracle rate (normally done by the oracle server)
    // Since we are using AAVE (instead of HAM as ETH/HAM doesn't exists),
  ...
  // Success scenarios
  describe("success", () => {
    /* A new instance of HAMToken, ETHHAMPriceFeedOracle, and HBank contracts are set before each test case.
    ...
    beforeEach(async () => {
      ...
      hBank = await HBank.new(hamToken.address, 10, oracle.address); // Sets the yearly return rate to 10
    ...
    it("should update the yearly return rate correctly", async () => {
      await hBank.updateYearlyReturnRate(15, {
        from: accounts[0],
      });
      assert.equal(await hBank.yearlyReturnRate(), 15);
    });
    ...
    it("should deposit correctly", async () => {
      ...
      expect(Number(await hBank.totalDepositAmount())).to.be.above(0);
    });
    ...
    it("should withdraw correctly", async () => {
      ...
      assert.equal(Number(await hBank.totalDepositAmount()), 10 ** 18);
    });
    ...
    it("should borrow correctly", async () => {
      ...
      const borrowerOldTokenBalance = Number(
        await hamToken.balanceOf(accounts[1])
      );
      const borrowerOldEthBalance = Number(
        await web3.eth.getBalance(accounts[1])
      );
      const bankOldEthBalance = Number(
        await web3.eth.getBalance(hBank.address)
      );
      const bankOldTokenBalance = Number(
        await hamToken.balanceOf(hBank.address)
      );

      // Borrows 0.0001 Ether while collateralizing (0.001 * colleteralization ratio * rate) HAM tokens == 0.0255 HAM tokens
      // Remember that ERC20 tokens also have 18 decimals. Thus, 0.0255 HAM tokens == 0.0255 * 10**18 units
      // Note: We have allowed for HBank to transfer 0.05 HAM tokens from borrower to itself,
      // while the collateral value is 0.0255 HAM tokens. Normally, you should only allow the required amount.
      await hBank.borrow(web3.utils.toWei("0.001", "ether"), {
        from: accounts[1], // Borrower = accounts[1]
      });

      const borrower = await hBank.borrowers(accounts[1]);
      ...
      // Checks if borrower has more ETH than before and less HAM tokens
      ...
      // Checks if HBank has more HAM tokens than before and less ETH
    ...
    it("should pay loan correctly", async () => {
      ...
      // Checks if borrower has more HAM tokens than before and less ETH
      const borrowerNewTokenBalance = Number(
        await hamToken.balanceOf(accounts[1])
      );
      const borrowerNewEthBalance = Number(
      ...
      // Checks if HBank has more ETH than before and less HAM tokens
      const bankNewEthBalance = Number(
        await web3.eth.getBalance(hBank.address)
      );
      const bankNewTokenBalance = Number(
  ...
  describe("failure", () => {
    it("should reject invalid yearly return rate", async () => {
      ...
      try {
        hamToken = await HAMToken.new();
        oracle = await ETHHAMPriceFeedOracle.new();
        hBank = await HBank.new(hamToken.address, 1000, oracle.address);
      } catch (e) {
        err = e;
      }
    ...
    it("should reject borrowing when HBank don't have enough Ether", async () => {
      let err;
      try {
        hamToken = await HAMToken.new();
        oracle = await ETHHAMPriceFeedOracle.new();
        ...
        await hBank.borrow(web3.utils.toWei("1", "ether"), {
          from: accounts[1],
        });
      } catch (e) {
        err = e;
